...
describe('summariseRule', () => {
    it('describes timing and conditions in plain language', () => {
        ...
        expect(summariseRule(fixture('rule_logo'), METRIC_CATALOG)).toBe('3 days after registration · Logo not uploaded');
        expect(summariseRule(fixture('rule_document'), METRIC_CATALOG)).toBe('In trial · No documents uploaded');
        expect(summariseRule(fixture('rule_trial_7'), METRIC_CATALOG)).toBe('7 days before trial expiry');
        expect(summariseRule(fixture('rule_inactive_14'), METRIC_CATALOG)).toBe('No portal sign-in for 14 days');
