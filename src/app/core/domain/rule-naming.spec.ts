...
describe('nextDuplicateName', () => {
  it('appends (Copy) when that name is unused', () => {
    expect(nextDuplicateName('Trial — 7 Days Remaining', ['Trial — 7 Days Remaining'])).toBe(
  ...
  it('increments the copy suffix when (Copy) already exists', () => {
    ...
    expect(nextDuplicateName('Trial — 7 Days Remaining', existing)).toBe(
      'Trial — 7 Days Remaining (Copy 2)',
    );
  });
  ...
  it('continues incrementing past later copy suffixes', () => {
    ...
    expect(nextDuplicateName('Welcome to Portal Genie', existing)).toBe(
      'Welcome to Portal Genie (Copy 3)',
    );
  });
});
